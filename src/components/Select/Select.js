...
class Select extends Component {
    ...
    generateOptions = () =>{
        const staticOptions = this.props.constOptions.map(el=> <div key={el.value} className="options__elements" data-geocode={el.geocode} data-value={el.value}>{el.label}</div>);

        let options;

        if (this.state.options.length >0) options = this.state.options.map(el =><div key={el.value} className="options__elements" data-geocode={el.geocode} data-value={el.value}>{el.label}</div>);
